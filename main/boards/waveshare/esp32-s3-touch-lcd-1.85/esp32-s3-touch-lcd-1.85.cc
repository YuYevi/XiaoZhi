...
#include "esp_io_expander_gpio_wrapper.h"   // 注册 TCA9554 虚拟 GPIO，使 ES8311 可经虚拟 GPIO 控制功放
...
private:
    ...
    void InitializeTca9554(void) {
        ...
        // 功放 PA(TCA9554 引脚5)设为输出，初始电平由 AUDIO_CODEC_PA_INVERTED 决定（默认低=静音）
        // 真正拉高由 ES8311 codec 在开启输出时通过虚拟 GPIO(AUDIO_CODEC_PA_PIN=55) 完成
        ret = esp_io_expander_set_dir(io_expander, BAJI185_IOX_PIN_MASK_PA, IO_EXPANDER_OUTPUT);
        ESP_ERROR_CHECK(ret);
        ret = esp_io_expander_set_level(io_expander, BAJI185_IOX_PIN_MASK_PA, AUDIO_CODEC_PA_INVERTED ? 1 : 0);
        ESP_ERROR_CHECK(ret);

        // 把 TCA9554 注册为虚拟 GPIO 控制器(TCA9554_GPIO_VIRTUAL_BASE=50)，
        // 否则 ES8311 codec 用 AUDIO_CODEC_PA_PIN(虚拟 GPIO 55) 控制功放时会失败
        ret = esp_io_expander_gpio_wrapper_append_handler(io_expander, TCA9554_GPIO_VIRTUAL_BASE);
        ESP_ERROR_CHECK(ret);
    }
...
public:
    ...
    virtual AudioCodec* GetAudioCodec() override {
        static Es8311AudioCodec audio_codec(
            ...
            AUDIO_I2S_GPIO_MCLK,            // 45
            AUDIO_I2S_SPK_GPIO_BCLK,        // 41
            AUDIO_I2S_SPK_GPIO_LRCK,        // 46
            AUDIO_I2S_SPK_GPIO_DOUT,        // 42
            AUDIO_I2S_MIC_GPIO_DIN,         // 40
            AUDIO_CODEC_PA_PIN,             // 虚拟 GPIO 55 (TCA9554 引脚5)
            AUDIO_CODEC_ES8311_ADDR,
            true, AUDIO_CODEC_PA_INVERTED); // use_mclk=true
